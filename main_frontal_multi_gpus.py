...
from train_frontal_multi_gpus import Trainer
import datetime
...
flags.DEFINE_float("sample_pose_interval", 0.1, 'test identity interval')  # epoch<sample<test
flags.DEFINE_float("test_pose_interval", 0.1, 'test pose interval save the decode images')
flags.DEFINE_float("epoch_interval", 0.1, 'validation interval save the decode images')
...
flags.DEFINE_string("data_path_txt", './multipie_txt/synthesis_msr/synthesis_msr_train_validation_refine20.txt', '')
flags.DEFINE_string("datapath", '/world/data-gpu-58/wangyuequan/data_sunkejia/Multi_session/', '')
flags.DEFINE_string("valid_path_txt", './multipie_txt/synthesis_msr/synthesis_msr_test.txt', '')
...
flags.DEFINE_string('model_name','frontalization','')
flags.DEFINE_string('newtag','msr_multipie_refine20_id_loss_syn','model version')
#如果GD不一区训练的话，会出现彩色块
...
flags.DEFINE_string("gpu_list",'3,6,7',"CUDA visiabTruele device")
...
def main():
    ...
    drgan = Trainer(sample_interval=FLAGS.epoch_interval, sample_pose_interval=FLAGS.sample_pose_interval,
                    ...
                    warmup=FLAGS.warmup, delta=FLAGS.delta, discribe=FLAGS.discribe,test_batch=FLAGS.sample_batch_size,
