...
from train_fineturne_fr import Trainer
import datetime
...
flags.DEFINE_integer("batch_size", 128, 'tarin_batch_size for one gpu')
...
flags.DEFINE_float("sample_pose_interval", 1, 'test identity interval')  # epoch<sample<test
flags.DEFINE_float("test_pose_interval", 1, 'test pose interval save the decode images')
flags.DEFINE_float("epoch_interval", 1, 'validation interval save the decode images')
...
flags.DEFINE_string("data_path_txt", './multipie_txt/imlist_totalsession_list_label_4_part_fr_train.txt', '')
flags.DEFINE_string("datapath", '/world/data-gpu-58/wangyuequan/data_sunkejia/Multi_session/', '')
flags.DEFINE_string("valid_path_txt", './multipie_txt/imlist_totalsession_list_label_4_part_fr_test.txt', '')
...
flags.DEFINE_integer('label_nums', 1, 'label numbers')
flags.DEFINE_integer('thread_nums', 10, 'data read thread nums')

flags.DEFINE_string('model_name','finetunefr','')
flags.DEFINE_string('newtag','finetun_resnet','model version')
#如果GD不一区训练的话，会出现彩色块
...
flags.DEFINE_integer('Gloop',1,'G:D,n:1')
flags.DEFINE_string("gpu_list",'2',"CUDA visiabTruele device")
...
# check_point='./logdir_caspeal/logdir_finetunefr/20171115_gpu_1_v0.2/checkpoint/finetunefr-2806'
# check_point='./tf_inception_v1/model-20171024-190413.ckpt-147982'
