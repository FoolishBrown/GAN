...
from train_model_8_25_totalcode import Trainer
import datetime
...
flags.DEFINE_integer("sample_batch_size", 14, 'test,sample batch_size for one gpu,一般比pose_c相关')  # 保证可以被2、gpu数整除
...
flags.DEFINE_integer('pose_code', 13, 'pose个数')
FLAGS = flags.FLAGS
flags.DEFINE_integer('mode', 2, 'GAN mode,2:mean MultiPIE')
flags.DEFINE_float("sample_pose_interval", 0.5, 'test identity interval')  # epoch<sample<test
flags.DEFINE_float("test_pose_interval", 0.5, 'test pose interval save the decode images')
flags.DEFINE_float("epoch_interval", 0.5, 'validation interval save the decode images')
...
flags.DEFINE_string("data_path_txt", './multipie_txt/multi_subjects_labeled_cut_train_oneshot_01_90.txt', '')
flags.DEFINE_string("datapath", '/world/data-gpu-58/wangyuequan/data_sunkejia/Multi_session/', '')
flags.DEFINE_string("valid_path_txt", './multipie_txt/multi_subjects_labeled_cut_test_oneshot_01_90.txt', '')
...
flags.DEFINE_string('model_name', 'DR_MultiPIE_shareGAN_totalangle_13.058', '')
flags.DEFINE_string('newtag', 'flip_multipie13.0586', 'model version')
# 如果GD不一区训练的话，会出现彩色块
flags.DEFINE_string('discribe','重新训练，90度+更为详细的Log','version discribe')
flags.DEFINE_integer('loss_type', 1, '#0:ad 1:percptual loss 2:ps idloss')
flags.DEFINE_integer('Gloop', 2, 'G:D,n:1')
...
check_point = '{}'.format('./logdir_caspeal/logdir_DR_MultiPIE_shareGAN_totalangle_13.058/20170830_gpu_1_vflip_multipie13.0584/checkpoint/DR_MultiPIE_shareGAN_totalangle_13.058-9986')
