...
def inference_recognition(inputs,classnum,reuse=True):
    ...
    with tf.variable_scope('recognation_fc',reuse=reuse):
        net=slim.fully_connected(logits,classnum,activation_fn=None,normalizer_fn=None,weights_initializer=tf.truncated_normal_initializer(stddev=0.02),scope='recognition_soft')
...
def Custom_netD_discriminator_psloss(image_input,posenum=7,reuse=False,illuminationnum=20):
    '''
    ...
    with tf.variable_scope('discriminator/psloss',reuse=reuse) as vs:
        ...
        with slim.arg_scope([slim.conv2d],normalizer_fn=slim.batch_norm,activation_fn=tf.nn.elu,padding='SAME',weights_initializer=tf.truncated_normal_initializer(stddev=0.02)):
            ...
            lightlogits = slim.fully_connected(slim.flatten(avgpool), illuminationnum, activation_fn=None, normalizer_fn=None,
                                            weights_initializer=tf.truncated_normal_initializer(stddev=0.02),
                                            scope='light_fc')

            pslogits = slim.fully_connected(slim.flatten(avgpool),posenum,activation_fn=None,normalizer_fn=None,weights_initializer=tf.truncated_normal_initializer(stddev=0.02),scope='ps_soft')
            return pslogits,lightlogits


def Custom_netD_discriminator_illuminationloss(image_input,illuminationnum=3,reuse=False):
    '''
    08-04修改，使用avgpool减小网络大小
    08-05修改，inst_norm 替换为batchnorm
    :param image_input:
    ...
    :return:
    '''
    with tf.variable_scope('discriminator/lightloss',reuse=reuse) as vs:
        kernel_size=[3,3]
        filter_num=32
        with slim.arg_scope([slim.conv2d],normalizer_fn=slim.batch_norm,activation_fn=tf.nn.elu,padding='SAME',weights_initializer=tf.truncated_normal_initializer(stddev=0.02)):
            #96
            ...
            net = slim.conv2d(net,filter_num*6,kernel_size,scope='conv8')
            #12
            net = slim.max_pool2d(net, [2, 2], scope='max3')
            net = slim.conv2d(net,filter_num*4,kernel_size,scope='conv10')
            net = slim.conv2d(net,filter_num*8,kernel_size,scope='conv11')
            #6
            net = slim.max_pool2d(net, [2, 2], scope='max4')
            net = slim.conv2d(net,filter_num*6,kernel_size,scope='conv13')
            net = slim.conv2d(net,filter_num*10,kernel_size,scope='conv14')
            #two path -feature -W Omegapredict_r_label
            #avg出来之后应该是1*1*320的tensor
            ...
            avgpool=slim.pool(net,[6,6],pooling_type="AVG",scope='avgpool')
            pslogits = slim.fully_connected(avgpool,illuminationnum,activation_fn=None,normalizer_fn=None,weights_initializer=tf.truncated_normal_initializer(stddev=0.02),scope='light_fc')
            return pslogits