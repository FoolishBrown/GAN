...
class Trainer(object):
    ...
    def _init_model(self):
        ...
        self.input_light = tf.placeholder(dtype=tf.int64,shape=[None],name='input_illumination')#illumination
        ...
        self.light_reverse = tf.concat(tf.split(self.light,2,axis=0)[::-1],axis=0)
    ...
    def _init_validation_model(self):
        ...
        with tf.name_scope('test'):
            ...
            self.slerp_code = tf.get_variable('slerpcode',
                                   shape=(self.test_slerp_count, 1, 1, 512 + self.pose_c + self.light_c),
                                   dtype=tf.float32)
    ...
    def validation(self,interval_i,epoch_n,step):
        ...
        for idx in xrange(sample_count):  # 将数据集中的同一个人所有!!!角度！！！照片都跑一次
            ...
            sample_data, sample_data_ex, encode_real, encode_syn1, encode_syn2 = self.sess.run([self.output_syn1, self.output_syn2,self.pidrcontent,self.encode_syn_1,self.encode_syn_2],
                                                   feed_dict={
    ...
    def slerp_interpolation(self,batch_image,batch_label,epoch,interval):
        ...
        encode_512 = self.sess.run(self.encode_slerp,
                             feed_dict={self.batch_data: batch_image[index_select],
                                        ...
                                        self.input_light: batch_label[2][index_select]})
        ...
        encode_519 = np.concatenate([np.reshape(encode_512, [-1, 512]), b_1], axis=1)
    ...
    def _predict_drgan_multipie(self,reuse=False):
        ...
        :return:
        '''
        _,self.logits,self.pidrcontent = nets.inference_recognition(self.batch_data,self.class_nums,reuse=reuse)
        self.output_en = tf.reshape(self.pidrcontent,[-1,1,1,512])

        pose_add_noise_onhot=tf.concat([self.pose,self.light],axis=1)
        pose_add_noise=tf.reshape(pose_add_noise_onhot,[-1,1,1,self.pose_c+self.light_c])
        ...
        pose_add_noise_onhot_reverse=tf.concat([self.pose_reverse,self.light_reverse],axis=1)
        pose_add_noise_reverse=tf.reshape(pose_add_noise_onhot_reverse,[-1,1,1,self.pose_c+self.light_c])
        pose_add_noise_reverse_float=tf.cast(pose_add_noise_reverse,dtype=tf.float32)
        sample_add_pn_ex = tf.concat([self.output_en, pose_add_noise_reverse_float], 3)
        #syn2
        ...
        self.real_poselogits,self.real_lightlogits=nets.Custom_netD_discriminator_psloss(self.batch_data,posenum=self.pose_c,reuse=reuse,illuminationnum=self.light_c)
        ...
        self.fake1_poselogits,self.fake1_lightlogits=nets.Custom_netD_discriminator_psloss(self.output_syn1,posenum=self.pose_c,reuse=True,illuminationnum=self.light_c)
        ...
        self.fake2_poselogits,self.fake2_lightlogits=nets.Custom_netD_discriminator_psloss(self.output_syn2,posenum=self.pose_c,reuse=True,illuminationnum=self.light_c)
    ...
    def _loss_gan_multipie(self):
        ...
        with tf.name_scope('D_loss'):
            ...
            self.ps_loss_real = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(
                labels=self.pose, logits=self.real_poselogits
            ))
            #illumination loss
            self.light_loss_real=tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(
                labels=self.light,logits=self.real_lightlogits
            ))
            #adversarial
            ...
            #content loss
            self.split_content = tf.split(self.pidrcontent, 2, axis=0)[::-1]
        ...
        with tf.name_scope('G_loss'):
            ...
            self.ps_loss_syn1 = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(
                labels=self.pose, logits=self.fake1_poselogits))
            self.ps_loss_syn2 = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(
                labels=self.pose_reverse, logits=self.fake2_poselogits))

            #illumination loss
            self.light_loss_syn1 = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(
                labels=self.light, logits=self.fake1_lightlogits))
            self.light_loss_syn2 = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(
                labels=self.light_reverse, logits=self.fake2_lightlogits))

            #identity preserved loss
            self.id_preserve_f1 = tf.reduce_mean(tf.abs(tf.subtract(self.pidrcontent, self.pidf1content)))
            self.id_preserve_f2 = tf.reduce_mean(tf.abs(tf.subtract(self.pidrcontent, self.pidf2content)))
        ...
        with tf.name_scope('loss_d'):
            ...
            tf.summary.scalar('light_loss_real',self.light_loss_real)
        with tf.name_scope('loss_g'):
            ...
            tf.summary.scalar('light_loss_syn1',self.light_loss_syn1)
            tf.summary.scalar('light_loss_syn2',self.light_loss_syn2)
        if True:
    ...
    def _loss_compute(self):#G:D 2:1
        ...
        :return:
        '''
        self.d_loss =1* (self.ad_loss_real +self.lr_alpha*self.ad_loss_fake1+(1-self.lr_alpha)*self.ad_loss_fake2)+\
                    self.ps_loss_real+\
                    self.light_loss_real
        # self.d_loss= 0.1 * self.id_loss_real
        # self.g_loss = (self.lr_alpha)*20*self.pixel_loss_f1+(1-self.lr_alpha)*20*self.pixel_loss_f2
        self.g_loss =(self.lr_alpha)*(10*self.pixel_loss_f1+self.ad_loss_syn1+self.id_preserve_f1+self.light_loss_syn1
                                      +self.ps_loss_syn1)+\
                     (1-self.lr_alpha)*(10*self.pixel_loss_f2+self.ad_loss_syn2+self.id_preserve_f2+self.light_loss_syn2
                                        +self.ps_loss_syn2)
